...
export const Settings: React.FC<SettingsProps> = ({ 
    clients: fullProfiles, templates, snippets, onUpdate, onClose, currentUser, users: teamMembers, branding 
}) => {
  ...
  // Branding State (Local state initialized from props for editing)
  const [brandingConfig, setBrandingConfig] = useState<AppConfig>(branding);
  ...
  // Theme Toggle Effect & Lazy Load Services
  useEffect(() => {
  ...
  const loadServicesAndRateCard = async () => {
      // Services are fetched from DB for the editor view as they contain HTML content not needed in App state
  ...
  return (
    ...
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in" onClick={onClose}>
        <div className="bg-white dark:bg-gray-800 rounded-[2rem] shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden animate-in zoom-in-95 w-[95vw] max-w-[1920px] mx-auto flex flex-col h-[92vh] relative" onClick={e => e.stopPropagation()}>
                                ...
                                {fullProfiles.map(client => (
                                    <div key={client.name} className="flex flex-col justify-between p-5 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-swave-purple/30 hover:shadow-lg hover:shadow-purple-500/5 transition-all group gap-4 relative overflow-hidden min-h-[140px]">
                                        ...
                                        <div className="flex gap-2 justify-end relative z-10 border-t border-gray-50 dark:border-gray-700/50 pt-3">
                                            ...
                                            <button onClick={async () => { if(confirm(`Remove ${client.name}?`)) { await db.removeClient(client.name); onUpdate(); }}} className="px-4 py-2 text-xs font-bold text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors">Remove</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
