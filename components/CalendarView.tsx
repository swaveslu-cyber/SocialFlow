            ...
            {[...Array(daysInMonth)].map((_, i) => {
                ...
                return (
                    <div key={day} className={`bg-white dark:bg-gray-900 flex flex-col relative transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 min-h-0 group overflow-hidden`}>
                        {/* Day Number */}
                        <span className={`absolute top-1.5 right-1.5 text-[10px] font-bold z-10 w-6 h-6 flex items-center justify-center rounded-full pointer-events-none transition-all ${isToday ? 'bg-swave-purple text-white shadow-md scale-110' : 'text-gray-400 group-hover:bg-gray-100 dark:group-hover:bg-gray-800'}`}>
                            {day}
                        </span>

                        {/* Events List - Internal Scroll if needed */}
                        <div className="flex-grow flex flex-col gap-1.5 overflow-y-auto custom-scrollbar p-1.5 pt-8">
                            {dayPosts.map(post => (
                                <div 
                                    key={post.id}
                                    onClick={(e) => {
                                    ...
                                    className={`
                                        w-full px-2 py-1.5 rounded-[6px] cursor-pointer transition-all hover:brightness-110 hover:translate-x-0.5
                                        flex items-center gap-2 overflow-hidden shrink-0
                                        ${getEventStyle(post.status)}
                                    `}
                                    title={`${post.client}: ${post.caption}`}
                                >
                                    <PlatformIcon platform={post.platform} className="w-3 h-3 flex-shrink-0 opacity-95" />
                                    
                                    {/* Text */}
                                    <span className="text-[10px] font-bold uppercase tracking-wide truncate leading-none text-white/95">
                                        {post.client}
                                    </span>
                                </div>
                            ))}
