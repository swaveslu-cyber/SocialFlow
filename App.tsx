...
import { 
  ...
  Smile, Loader2, ArrowRight, RotateCcw, ChevronDown, Building2, Coffee, BookOpen, BarChart3, Inbox, CheckCheck, DollarSign
} from 'lucide-react';
...
export default function App() {
  ...
  const loadData = async (silent: boolean = false) => {
    ...
    try {
      ...
      const [fetchedPosts, fetchedClients, fetchedCampaigns, fetchedTemplates, fetchedSnippets, fetchedBranding, fetchedInvoices, fetchedUsers, fetchedServices] = await Promise.all([
        db.getAllPosts(), db.getClients(), db.getCampaigns(), db.getTemplates(), db.getSnippets(), db.getAppConfig(), db.getInvoices(), db.getUsers(), db.getServices()
      ]);
                ...
                <div className="bg-white dark:bg-gray-900 w-full max-w-5xl h-[90vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-2 gap-12">
                        <div className="space-y-8">
                            ...
                            <div className="space-y-2"><label className="text-xs font-bold text-gray-400 uppercase">Campaign</label><input list="campaigns" value={newPostCampaign} onChange={e => setNewPostCampaign(e.target.value)} className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl font-bold border-none" placeholder="e.g. Q1 Launch"/><datalist id="campaigns">{Array.from(new Set(posts.map(p => p.campaign))).map(c => <option key={c} value={c!} />)}</datalist></div>
                            ...
                            <div className="space-y-2"><label className="text-xs font-bold text-gray-400 uppercase">Asset</label><div className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-3xl p-8 text-center relative group">{newPostMediaUrl ? (newPostMediaType === 'video' ? <video src={newPostMediaUrl} className="w-full max-h-64 object-contain" controls /> : <img src={newPostMediaUrl} className="w-full max-h-64 object-contain rounded-xl" />) : <div className="text-gray-400 flex flex-col items-center"><UploadCloud className="w-12 h-12 mb-2"/><p className="text-xs font-bold">Upload Media</p></div>}<input type="file" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer"/></div></div>
                        </div>
